...
pub async fn login(mut db: Connection<WebshopDatabase>, cookiejar: &CookieJar<'_>, login: Form<LoginData<'_>>) -> Result<(), Status> {
    let user_row = sqlx::query!("SELECT `user_id`, `email`, `password`, `is_admin` FROM `user` WHERE `email` = ?", login.email)
        .fetch_one(&mut *db).await
        .map_err(|_| Status::from_code(400).unwrap())?;
    if bcrypt::verify(login.password, &user_row.password).map_err(|_| Status::from_code(500).unwrap())? {
